...
class MainActivity : ComponentActivity() {
    ...
    private fun execute(action: SuggestedAction, text: String, results: List<ExtractionResult>) {
        ...
        when (action.type) {
            ...
            SuggestedActionType.MAPS -> launch(
            ...
            SuggestedActionType.OPEN_LINK -> first(results, ExtractionType.URL)
                ?.let { launch(Intent(Intent.ACTION_VIEW, Uri.parse(normalizeUrl(it)))) }
                ?: fail("No link was found.")
            SuggestedActionType.EMAIL -> first(results, ExtractionType.EMAIL)
                ?.let { launch(Intent(Intent.ACTION_SENDTO, Uri.parse("mailto:${Uri.encode(it)}"))) }
                ?: fail("No email address was found.")
            SuggestedActionType.CALL -> first(results, ExtractionType.PHONE)
                ?.let { launch(Intent(Intent.ACTION_DIAL, Uri.parse("tel:${Uri.encode(it)}"))) }
    ...
    private fun launch(intent: Intent) {
        try {
            if (intent.resolveActivity(packageManager) == null) fail("No compatible app is installed for this action.")
    ...
    private fun first(results: List<ExtractionResult>, type: ExtractionType): String? =
        results.firstOrNull { it.type == type }?.value
    ...
    private enum class Screen { HOME, TEXT_ENTRY, LINK_ENTRY, PREVIEW, EXTRACTION, ACTIONS, SAVE, CHECKLIST }
    ...
    Scaffold(topBar = { TopAppBar(title = { Text("Import preview") }) }) { padding ->
        LazyColumn(
            ...
            item {
                OutlinedTextField(
                    value = editable,
                    onValueChange = { editable = it.take(SharedTextParser.MAX_SHARED_TEXT_LENGTH) },
                    modifier = Modifier.fillMaxWidth(),
...
private fun ExtractionScreen(
    ...
    Scaffold(topBar = { TopAppBar(title = { Text("Extracted information") }) }) { padding ->
        LazyColumn(
            ...
            item {
                Text(
                    if (results.isEmpty()) "Nothing specific was detected" else "${results.size} useful details found",
...
private fun ChecklistScreen(value: String, onBack: () -> Unit, onSave: (String) -> String?) {
    val suggested = value.lineSequence().map(String::trim).filter(String::isNotBlank)
        .joinToString("\n") { "☐ ${it.trimStart('-', '•', ' ')}" }
...
private fun HomeScreen(
    ...
    Scaffold(topBar = { TopAppBar(title = { Text("Drop") }) }) { padding ->
        LazyColumn(
            ...
            if (references.isEmpty()) item { Text("No saved items yet") }
            else items(references, key = SavedReference::id) { reference ->
                Card(Modifier.fillMaxWidth()) {
                    Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(6.dp)) {
...
private fun ExtractionType.label(): String = when (this) {

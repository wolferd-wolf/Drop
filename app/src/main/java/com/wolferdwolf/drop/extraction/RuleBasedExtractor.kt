...
object RuleBasedExtractor {
    ...
    private val rules = listOf(
        ...
        Rule(ExtractionType.DATE, Regex("(?i)\\b(?:0?[1-9]|[12]\\d|3[01])(?:st|nd|rd|th)?[-\\s]+(?:jan(?:uary)?|feb(?:ruary)?|mar(?:ch)?|apr(?:il)?|may|jun(?:e)?|jul(?:y)?|aug(?:ust)?|sep(?:t(?:ember)?|tember)?|oct(?:ober)?|nov(?:ember)?|dec(?:ember)?)\\.?(?:[-,\\s]+'?\\d{2,4})?\\b"), 0.93f),

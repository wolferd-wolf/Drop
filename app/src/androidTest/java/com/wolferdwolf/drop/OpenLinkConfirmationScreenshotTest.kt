...
class OpenLinkConfirmationScreenshotTest {
    ...
    fun captureEditableOpenLinkConfirmation() {
        ...
        ActivityScenario.launch<OpenLinkConfirmationActivity>(intent).use {
            ...
            assertNotNull(device.wait(Until.findObject(By.text("Open link")), 20_000L))
            assertNotNull(device.wait(Until.findObject(By.text("Confirm the website")), 5_000L))
            assertNotNull(device.wait(Until.findObject(By.text("Continue to Browser")), 5_000L))

...
class MissingCalendarDataFlowTest {
    @Test
    fun eventWithoutDateSuppressesCalendarSuggestionAndKeepsManualPath() {
        ActivityScenario.launch(MainActivity::class.java).use {
            ...
            input.text = "Team meeting in Wolf Hall. Agenda: launch review."
            ...
            assertVisibleAfterScroll(device, "Save reference", "Event-like content must still offer Save reference")
            assertVisibleAfterScroll(device, "Choose another action", "Manual action path must remain visible")
            assertFalse(
                "Calendar must not be suggested when no date was extracted",
                device.hasObject(By.text("Add calendar event"))
            )
            capture(device, "/data/local/tmp/drop-calendar-missing-date-suppressed.png")
        }
    }
